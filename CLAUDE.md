...
**Init order:** deps → preflight → kubeadm init → Gateway API CRDs → Cilium CNI → Longhorn storage → summary.
...
- `src/cni/` — Cilium CNI installation (prefers `cilium` CLI, falls back to Helm). Also handles Gateway API CRD installation (server-side apply) and enabling `gatewayAPI.enabled` in Cilium.
- `src/storage/` — Longhorn distributed block storage installation via Helm.
...
- Gateway API CRDs use `kubectl apply --server-side=true` because the CRDs are too large for client-side apply.
